<nav class="w-full border-b border-stone-200/60 bg-canvas">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-8">
            <!-- Corrected routing for Home -->
            <a href="{{ route('home') }}" class="text-stone-700 hover:text-brand font-medium transition-colors">Home</a>
            
            <!-- Corrected routing for Menu -->
            <a href="{{ route('menu') }}" class="text-stone-700 hover:text-brand font-medium transition-colors">Menu</a>
        </div>
        
        <div class="flex-1 flex justify-end">
            <!-- Note: If you have a login route, you would change this href to "{{ route('login') }}" -->
            <a href="/login" class="bg-brand hover:bg-[#732d2d] text-white font-medium text-sm px-6 py-2 rounded-sm transition-colors duration-200 shadow-sm">
                Login
            </a>
        </div>
