...
<body class="bg-canvas text-stone-900 font-sans antialiased">
    ...
    <header class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 lg:py-24">
        <div class="flex flex-col-reverse md:flex-row items-center gap-10 md:gap-12 lg:gap-16">
            ...
            <div class="w-full md:w-1/2 flex flex-col items-start text-left relative z-10">
                <h1
                    class="font-heading text-7xl md:text-8xl lg:text-9xl font-bold uppercase leading-none tracking-tight text-stone-900 mb-6 md:mb-8">
                    Our.<br>
            ...
            <div class="w-full md:w-1/2 relative">
                <div
                    class="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-canvas to-transparent z-10 hidden md:block">
                </div>

                <img src="{{ asset('static-images/Group 16.png') }}" alt="Delicious Menu Selection"
    ...
    <div
        class="w-full border-y-2 border-[#A44D49] bg-[#F8F6F1] py-3.5 overflow-hidden block relative left-0 right-0 m-0 p-0">
   ...
   <script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function updateSpeed() {
            ...
            if (screenWidth < 768) {
                speed = 0.5; // Slower speed for mobile devices
            } else if (screenWidth < 1024) {
                speed = 0.8; // Medium speed for tablets
            } else {
                speed = 1.2; // Original speed for desktop
            }
        }

        function setupMarquee() {
            // Adjust the speed before setting up the track
            updateSpeed();
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 lg:py-24">
        ...
        @if (session('success'))
            <div
                class="mb-8 rounded-xl border border-green-200 bg-green-50 px-5 py-4 text-green-700 text-sm font-medium">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="flex flex-col lg:flex-row gap-10 md:gap-12 lg:gap-14 xl:gap-16 items-start">
            ...
            <div class="order-last lg:order-first flex-1 w-full">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                    ...
                    @forelse ($items as $item)
                        <div
                            class="flex flex-col bg-white rounded-xl overflow-hidden shadow-sm border border-stone-200/60 transition-transform hover:-translate-y-1 group">
            ...
            <aside
                class="order-first lg:order-last w-full lg:w-[380px] xl:w-[420px] shrink-0 lg:sticky lg:top-8 space-y-6">

                <!-- CART CARD -->
                <div
                    class="bg-white rounded-2xl shadow-2xl shadow-brand/10 border-t-4 border-t-brand p-6 lg:p-8 relative overflow-hidden">

                    <div
                        class="absolute top-0 right-0 w-32 h-32 bg-brand/5 rounded-full -translate-y-10 translate-x-10">
                    </div>
                            ...
                            @foreach ($cart as $cartId => $cartItem)
                                <div class="flex gap-3 border-b border-stone-100 pb-4">
                                    <div class="w-16 h-16 rounded-lg overflow-hidden bg-stone-100 shrink-0">
                                        <img src="{{ $getMenuPhoto($cartItem['photo'] ?? null) }}"
                                            alt="{{ $cartItem['name'] }}" class="w-full h-full object-cover">
                                    </div>
                ...
                <div
                    class="bg-white rounded-2xl shadow-xl shadow-brand/10 border border-stone-100 p-6 lg:p-8 relative overflow-hidden">
