                ...
                <div class="space-y-6">
                    ...
                    @foreach ($orders as $order)
                        <div class="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
                            ...
                            <div class="p-4 md:p-6 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">

                                <div>
                                    <div class="flex flex-wrap items-center gap-2 md:gap-3">
                                        <h2 class="font-heading text-xl md:text-2xl font-bold uppercase text-stone-900">
                                            Order #{{ $order->id }}
                                        </h2>

                                        <span class="inline-flex items-center rounded-full border px-2 md:px-3 py-1 text-[10px] md:text-xs font-bold uppercase {{ $statusClass($order->status) }}">
                                            {{ $order->status }}
                                        </span>
                                    </div>

                                    <p class="text-xs md:text-sm text-stone-500 mt-1 md:mt-2">
                                        Ordered on {{ $order->created_at->format('M d, Y h:i A') }}
                                    </p>
                                </div>

                                <div class="text-left sm:text-right border-t sm:border-0 border-stone-100 pt-3 sm:pt-0 mt-1 sm:mt-0">
                                    <p class="text-[11px] md:text-xs text-stone-500 font-medium uppercase">
                                        Total Amount
                                    </p>

                                    <p class="text-lg md:text-xl font-bold text-brand">
                            ...
                            <div class="p-4 md:p-6 space-y-5 md:space-y-4">
                                ...
                                @forelse ($order->menuItems as $menuItem)
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-3 md:gap-4">
                                        ...
                                        <div class="w-full sm:w-24 h-40 sm:h-24 rounded-xl overflow-hidden bg-stone-100 shrink-0">
                                            <img
                                        ...
                                        <div class="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                            ...
                                            <div>
                                                ...
                                                @if (!empty($menuItem->menu_category))
                                                    <p class="text-[10px] md:text-[11px] uppercase tracking-wide text-brand font-bold mt-1">
                                            ...
                                            <p class="font-bold text-stone-900 text-lg md:text-base mt-1 sm:mt-0">
                            ...
                            <div class="px-4 md:px-6 py-3 md:py-4 bg-canvas/60 border-t border-stone-100">
                                @if ($order->status === 'pending')
                                    <p class="text-xs md:text-sm text-stone-600">
                                ...
                                @elseif ($order->status === 'processing')
                                    <p class="text-xs md:text-sm text-purple-700 font-medium">
                                        Your food is being prepared.
                                    </p>
                                @elseif ($order->status === 'completed')
                                    <p class="text-xs md:text-sm text-green-700 font-medium">
